...
export function Juniores() {
  ...
  useEffect(() => {
    async function fetchAlunosJuniores() {
      try {
        const response = await fetch('http://localhost:3000/alunos/juniores');
        if (response.ok) {
          const data = await response.json();
          setAlunos(data);
        } else {
          console.error('Erro ao buscar alunos do juniores');
        }
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('http://localhost:3000/chamada/juniores', {
        method: 'POST',
        headers: {
              ...
              {alunos.map((aluno, index) => (
                <div key={aluno.id} className="_cnt">
                  ...
                  <Checkbox
                    ...
                    checked={aluno.presente === "presente"}
